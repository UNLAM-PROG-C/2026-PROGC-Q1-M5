...
public class AudioManager
{
  ...
  private static final double DEFAULT_VOLUME = 0.75;
  private static final int PCM_SAMPLE_SIZE_BITS = 16;
  private static final int PCM_BYTES_PER_CHANNEL = 2;
  private static final double DB_VOLUME_SCALE_FACTOR = 20.0;
  private static final float SILENCE_GAIN_DB = -80f;
  ...
  private AudioInputStream toPlayableStream(AudioInputStream audioInputStream)
 {
    ...
    if (format.getEncoding() == AudioFormat.Encoding.PCM_SIGNED)
  {
    ...
    AudioFormat decodedFormat = new AudioFormat(
      ...
      format.getChannels(),
      format.getChannels() * PCM_BYTES_PER_CHANNEL,
      format.getSampleRate(),
      false
    );
  ...
  private float effectiveGain(float baseGainDb, double volume)
 {
    if (volume <= 0.0)
    {
      return SILENCE_GAIN_DB;
    }
    return baseGainDb + (float) (DB_VOLUME_SCALE_FACTOR * Math.log10(volume));
  }
